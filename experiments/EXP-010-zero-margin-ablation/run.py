...
NS=(8,12,16,20,24);TARGET=128;MAXC=200000
...
def execute():
 checked=0
 for n in NS:
  rng=random.Random(1000000+n);srng=random.Random(1010000+n)
  ...
  while el<TARGET and gen<MAXC:
   ...
   for i in split:
    ...
    for j,o in enumerate(schedules):
     checked+=1;f,sw,conv=run(st,w,o)
     if not conv or (f!=q and f!=ps[minority]):
      ...
      return {'experiment':'EXP-010','result':'COUNTEREXAMPLE_FOUND','N':n,'generated_candidates':gen,'eligible_index':el,'checked_trajectories':checked,'patterns':[list(p) for p in ps],'Q':list(q),'Q_margins':margins,'coordinate':i+1,'minority_index':minority+1,'schedule_kind':'cyclic' if j<n else 'random','schedule_index':j+1,'order':[x+1 for x in o],'final':list(f),'classification':cls,'sweeps':sw}
  if el<TARGET:return {'experiment':'EXP-010','result':'UNCERTAIN','reason':'INSUFFICIENT_ELIGIBLE','N':n,'generated_candidates':gen,'eligible':el}
 return {'experiment':'EXP-010','result':'NO_COUNTEREXAMPLE_IN_SEARCH','checked_trajectories':checked}
