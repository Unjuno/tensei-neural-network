            ...
            for i in split:
                ...
                for j,order in enumerate(schedules):
                    ...
                    if not conv or (final!=q and final not in minority):
                        ...
                        return {"experiment":"EXP-009","result":"COUNTEREXAMPLE_FOUND","N":n,"generated_candidates":generated,"eligible_index":eligible,"checked_trajectories":checked,"patterns":[list(p) for p in ps],"Q":list(q),"coordinate":i+1,"minority_indices":[k+1 for k,p in enumerate(ps) if p[i]!=q[i]],"initial":list(st),"schedule_kind":"cyclic" if j<n else "random","schedule_index":j+1,"order":[x+1 for x in order],"final":list(final),"classification":cls,"sweeps":sweeps,"converged":conv}
        if eligible<TARGET:return {"experiment":"EXP-009","result":"UNCERTAIN","N":n,"generated":generated,"eligible":eligible}
    return {"experiment":"EXP-009","result":"NO_COUNTEREXAMPLE_IN_SEARCH","checked_trajectories":checked}
