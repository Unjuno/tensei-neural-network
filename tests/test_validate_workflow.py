...
class WorkflowValidatorTest(unittest.TestCase):
    ...
    def test_gate_candidate_passes_strict_when_ready(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/README.md").write_text("状態: GATE_CANDIDATE\n", encoding="utf-8")
        self.assertEqual(validator.run(r, strict=True), 0)

    def test_gate_candidate_uses_full_gate_checks(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/README.md").write_text("状態: GATE_CANDIDATE\n", encoding="utf-8")
        (r / "experiments/chapters/001/semantic-review.md").write_text("状態: `FAIL`\n", encoding="utf-8")
        self.assertIn("WF019", self.codes(r))

    def test_in_progress_still_fails_strict(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/README.md").write_text("状態: IN_PROGRESS\n", encoding="utf-8")
        self.assertIn("WF060", self.codes(r))
        self.assertEqual(validator.run(r, strict=True), 1)

    def test_missing_chapter_package_fails(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/README.md").unlink()
        self.assertIn("WF010", self.codes(r))

    def test_missing_mandatory_verification_fails(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/verification.md").unlink()
        self.assertIn("WF013", self.codes(r))

    def test_gate_requires_pass_verification(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/verification.md").write_text("状態: `FAIL`\n", encoding="utf-8")
        self.assertIn("WF014", self.codes(r))

    def test_gate_requires_semantic_review(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/semantic-review.md").unlink()
        self.assertIn("WF018", self.codes(r))

    def test_gate_requires_pass_semantic_review(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/semantic-review.md").write_text("状態: `UNCERTAIN`\n", encoding="utf-8")
        self.assertIn("WF019", self.codes(r))
    ...
    def test_unverified_table_row_fails_gate(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/terminology.md").write_text(
            "| 原概念 | 表記 | 人物発話 | 状態 | 根拠 |\n"
            "|---|---|---|---|---|\n"
            "| term | 候補 | 未確認 | UNVERIFIED | none |\n",
            encoding="utf-8",
        )
        self.assertIn("WF011", self.codes(r))

    def test_missing_event_reference_fails(self) -> None:
        r = self.make_fixture()
        (r / "novel/chapters/001-outline.md").write_text("採用event:\n\nEVT-999\n", encoding="utf-8")
        self.assertIn("WF020", self.codes(r))

    def test_forbidden_future_event_is_not_dependency(self) -> None:
        self.assertNotIn("WF020", self.codes(self.make_fixture()))

    def test_duplicate_event_id_fails(self) -> None:
        r = self.make_fixture()
        (r / "novel/events/EVT-001-two.md").write_text("# duplicate\n", encoding="utf-8")
        self.assertIn("WF030", self.codes(r))

    def test_old_gate_name_warns(self) -> None:
        r = self.make_fixture()
        (r / "experiments/chapters/001/README.md").write_text("状態: PREPUBLICATION_VERIFIED\n", encoding="utf-8")
        self.assertIn("WF061", self.codes(r))

    def test_obsolete_term_reintroduction_fails(self) -> None:
        r = self.make_fixture()
        (r / "novel/chapters/001.md").write_text("《フィクスト・ポイント》\n", encoding="utf-8")
        self.assertIn("WF050", self.codes(r))
